...
- **Zero regressions**: the entire DataHub layer is env-gated; the base platform runs identically without it — verified by a 55-check headless AppTest suite plus a pytest suite that grew from 10 to 47 checks without breaking one of the original ten.
